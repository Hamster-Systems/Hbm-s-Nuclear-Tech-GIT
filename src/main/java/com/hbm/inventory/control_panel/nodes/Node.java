...
public abstract class Node {
	...
	public void render(float mX, float mY, boolean isActive, boolean isSelected) {
		...
		if (isActive || isSelected) {
			...
			RenderHelper.drawGuiRectBatchedColor(posX - edge, posY - edge, 0, 0.875F, 40 + edge * 2, 6 + edge * 2,
					0.625F, 0.96875F, color[0], color[1], color[2], 1);
			RenderHelper.drawGuiRectBatchedColor(posX - edge, posY + 6, 0, 0.96875F, 40 + edge * 2, size + edge, 0.625F,
					0.984375F, color[0], color[1], color[2], 1);
			RenderHelper.drawGuiRectBatchedColor(posX - edge, posY + 6 + size, 0, 0.984375F, 40 + edge * 2, 1 + edge,
					0.625F, 1, color[0], color[1], color[2], 1);
		...
		RenderHelper.drawGuiRectBatchedColor(posX, posY + 1, 0, 0.125F, 40, 5, 0.625F, 0.203125F, color[0], color[1],
				color[2], 1);
		Tessellator.getInstance().draw();
